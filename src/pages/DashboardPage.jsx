...
function StatCard({ title, value, sub, icon: Icon, color, iconBg }) {
  return (
    <div className={`${CARD} p-4`}>
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs text-[#7A89AB] font-medium">{title}</p>
          <p className={`text-2xl font-bold mt-1 tracking-tight ${color}`}>{value}</p>
          {sub && <p className="text-xs text-[#7A89AB] mt-0.5">{sub}</p>}
        </div>
  ...
  return (
    <div className="p-4 space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard title="정상 설비" value={normalCount} sub={`전체 ${equipmentList.length}대 중`} icon={CheckCircle} color="text-emerald-600" iconBg="bg-emerald-50" />
        <StatCard title="고장 설비" value={faultCount} sub="즉시 조치 필요" icon={AlertTriangle} color="text-red-600" iconBg="bg-red-50" />
        <StatCard title="경고 설비" value={warningCount} sub="점검 권고" icon={Activity} color="text-amber-500" iconBg="bg-amber-50" />
        <StatCard title="점검 중" value={maintenanceCount} sub="예방점검 진행" icon={Wrench} color="text-[#3D6FE0]" iconBg="bg-[#EFF6FF]" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className={`${CARD} p-4`}>
          <h3 className="text-sm font-semibold text-[#0F1B33] mb-3 flex items-center gap-1.5">
            <Zap size={15} className="text-[#3D6FE0]" />
            시간별 부하율 추이 (오늘)
          </h3>
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={hourlyLoadData}>
              ...
              <Tooltip formatter={(v) => `${v}%`} contentStyle={{ borderColor: '#DDE2EE', borderRadius: 8, fontSize: 12 }} />
              ...
              <Line type="monotone" dataKey="부하율" stroke="#3D6FE0" strokeWidth={2} dot={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className={`${CARD} p-4`}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={alertTrendData}>
              ...
              <Tooltip contentStyle={{ borderColor: '#DDE2EE', borderRadius: 8, fontSize: 12 }} />
      ...
      <div className={`${CARD} p-4`}>
        <h3 className="text-sm font-semibold text-[#0F1B33] mb-3">활성 알림</h3>
        <div className="space-y-2">
          {activeAlerts.map(alert => (
            <div key={alert.id} className={`flex items-start gap-3 p-3 rounded-lg border ${
              ...
              <span className="text-xs text-[#7A89AB] flex-shrink-0">{alert.timestamp.split(' ')[1]}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className={`${CARD} p-4`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {equipmentList
            ...
            .slice(0, 6)
            .map(eq => (
              <div key={eq.id} className="flex items-center justify-between p-2.5 bg-[#F8F9FC] rounded-lg border border-[#DDE2EE]">
                <div>
                  <p className="text-xs font-semibold text-[#0F1B33]">{eq.name}</p>
                  <p className="text-xs text-[#7A89AB]">{eq.id}</p>
                </div>
                <div className="text-right">
                  <span className={`text-sm font-bold ${eq.temp >= 90 ? 'text-red-600' : eq.temp >= 70 ? 'text-amber-500' : 'text-emerald-600'}`}>
                    {eq.temp}°C
                  </span>
